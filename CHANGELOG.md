...
### Changed
- **Detection-resolution logic now delegates to bugspot's `ScaledDetector`.**
  `VideoInferenceProcessor` no longer reimplements the downscale/param-scaling/
  bbox-mapping policy; `setup()` builds a `ScaledDetector` and `process_frame()`
  calls it. This keeps behaviour identical to the `DetectionPipeline` path and
  removes duplicated policy that could drift between repos.

### Fixed
- **Fewer false positives when `detection_resolution` is set.** Via the shared
  `ScaledDetector`, `morph_kernel_size` is now scaled by the downscale factor
  (previously only `min_density` was). The fixed-size kernel had been acting
  ~1/scale larger on the downscaled frame and merging scattered motion into
  compact blobs that passed the filters. Requires `bugspot >=0.4.1`.
